...
Sign up at [cheese.example.com/developers](https://cheese.example.com/developers). Your API key appears on the dashboard immediately. It looks like this:
...
```bash
curl -s 'https://cheese.example.com/v2/cheeses?limit=3' \
...
```bash
curl -X POST 'https://sandbox.cheese.example.com/v2/orders' \
